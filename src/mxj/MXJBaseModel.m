...
- (NSDictionary *)modelToDictionary
{
    ...
    for (i = 0; i < propCount; i++)
    {
        ...
        if(propName)
        {
            ...
            else if ([obj isKindOfClass:[MXJBaseModel class]])
            {
                id subDic = [((MXJBaseModel *)obj) modelToDictionary];
                if (subDic)
                    [retDic setObject:subDic forKey:name];
            }
        }
    }
    free(properties);
